"""Raw FMEA seed facts for the knowledge graph — pure data, no I/O.

The 104 hand-authored electronics fault records (6 FMEA categories) migrated
verbatim from the legacy ad-hoc Neo4j seeder. This module is ONLY data: the
:class:`FaultRecord` tuples and their category grouping. The ontology mapping
(stable IDs, unified InstrumentKind/FaultKind/FaultCategory resolution,
entity/relation shapes) lives in :mod:`ate_cloud.services.kg_seed_facts`, and
graph persistence lives in :mod:`ate_cloud.services.kg_seeder`.

Categories (the unified ``FaultCategory`` vocab values):
...
# allow: SIZE_OK — pure seed-data table (104 fault facts, one tuple per fact).
"""
...
class FaultRecord:
    """A single raw FMEA fault fact (pre-ontology mapping).

    Carries the bilingual symptom/cause/solution text, the affected
    component, product type, triggered error code, legacy component bucket,
    legacy free-text diagnostic instrument, and the seed category. The
    ontology mapping in ``kg_seed_facts`` resolves ``category`` →
    ``FaultCategory``, ``instrument`` → ``InstrumentKind`` and ``error_code``
    → a canonical ``FaultKind`` and stable entity IDs.
    ...
    Attributes:
        ...
        component_type: Legacy component bucket (IC, Bus, Sensor, Connector, ...).
        product_type: Product type where symptom occurs.
        error_code: Unique error code triggered by the symptom.
        category: FMEA category (one of the 6 CAT_* constants).
        instrument: Legacy free-text diagnostic instrument name.
    """
...
def _build_fault_records() -> list[FaultRecord]:
    """Build the full list of 104 electronics fault records across 6 categories."""
    records: list[FaultRecord] = []

    def add(category: str, rows: list[tuple[str, ...]]) -> None:
        for sz, se, cz, ce, solz, sole, comp, ct, prod, ec, inst in rows:
            records.append(FaultRecord(
                ...
                category=category, instrument=inst,
            ))
...
FAULT_RECORDS: list[FaultRecord] = _build_fault_records()


__all__ = ["CAT_ASSEMBLY", "CAT_COMM", "CAT_ENVIRONMENT", "CAT_MIXED",
           "CAT_PASSIVE", "CAT_POWER", "FAULT_RECORDS", "FaultRecord"]
